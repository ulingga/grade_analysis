...
```{r}
# Use only the paper columns for PCA
...
# Convert to Real GPA Scale Using Existing mean_sd_pg
...
fit_300 <- brm(
  ...
  data = df_300,
  family = gaussian(),
  prior = priors,    # reuse priors from earlier
...
## Task - Correlation/Prediction: Stage III vs. Stage II + MEDSCI 142 - which one is a stronger predictor

Using Hierarchical regression to compare predictive strength step by step
Bayesian $R^2$ gives a transparent, uncertainty-aware summary of model fit, matching the Bayesian regression framework.
...
## Follow-up tasks (By Rachelle) - Explore which Stage II paper is the strongest predictor for MEDSCI 301 and 302.

Bayesian Model Averaging: Note this model only check data points that are not NA for all predictors.

```{r}
# Using Bayesian Model Averaging - around 25 students are included in the model

library(BAS)
bas.fit_301 <- bas.lm(
  MEDSCI_301 ~ MEDSCI_203 + MEDSCI_205 + MEDSCI_202 + MEDSCI_204,
  data = df_ug,
  modelprior = uniform(),       # This is the correct argument and function
  method = "BAS"
)

bas.fit_302 <- bas.lm(
  MEDSCI_302 ~ MEDSCI_203 + MEDSCI_205 + MEDSCI_202 + MEDSCI_204,
  data = df_ug,
  modelprior = uniform(),       # This is the correct argument and function
  method = "BAS"
)
...
```
Check for numbers of data points in each model
```{r}
# Get the formula used for bas.lm
formula_301 <- MEDSCI_301 ~ MEDSCI_203 + MEDSCI_205 + MEDSCI_202 + MEDSCI_204
formula_302 <- MEDSCI_302 ~ MEDSCI_203 + MEDSCI_205 + MEDSCI_202 + MEDSCI_204

# Count the complete cases included in the model
model_frame_301 <- model.frame(formula_301, data = df_ug)
n_obs_301 <- nrow(na.omit(model_frame_301))
cat("Number of complete cases for MEDSCI_301 model:", n_obs_301, "\n")

model_frame_302 <- model.frame(formula_302, data = df_ug)
n_obs_302 <- nrow(na.omit(model_frame_302))
cat("Number of complete cases for MEDSCI_302 model:", n_obs_302, "\n")
...
results <- tibble::tribble(
  ~Stage3,     ~Top_Stage2,   ~Posterior_Inclusion_Probability,   ~Top_Model_R2, ~Top_Model_Posterior_Probability,
  "MEDSCI 301", "MEDSCI 203", 0.87,    0.62,          0.25,
  "MEDSCI 301", "MEDSCI 205", 0.40,    0.67,          0.18,
  "MEDSCI 302", "MEDSCI 205", 0.98,    0.81,          0.27,
  "MEDSCI 302", "MEDSCI 204", 0.99,    0.82,          0.09
)
...
Regularised Regression (LASSO, Ridge, Elastic Net)
```{r}
library(glmnet)
# Combine predictors and response into a single dataframe
df_lasso <- df_ug[, c("MEDSCI_301", "MEDSCI_203", "MEDSCI_205", "MEDSCI_202", "MEDSCI_204")]

# Keep only complete cases (no NA in any relevant column)
df_lasso_complete <- na.omit(df_lasso)

# Prepare x and y
x <- as.matrix(df_lasso_complete[, c("MEDSCI_203", "MEDSCI_205", "MEDSCI_202", "MEDSCI_204")])
y <- df_lasso_complete$MEDSCI_301
...
Multiple imputation with mice + BMA handling missing data:

```{r}

# Check skewness for MEDSCI 301
library(moments)
skew_val <- skewness(df_ug$MEDSCI_301, na.rm = TRUE)
print(skew_val)

# Set up prior for skew-normal likelihood
priors <- c(
  prior(normal(0, 5), class = "Intercept"),
  prior(normal(0, 5), class = "b"),
  prior(normal(0, 2), class = "sigma"),
  prior(normal(-1.5, 0.5), class = "alpha")
)
...
Sys.setenv(SDKROOT = "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk")

# Specify which predictors can have missing data and use the mi() function
library(brms)

fit_brms_301 <- brm(
  bf(MEDSCI_301 ~ mi(MEDSCI_203) + mi(MEDSCI_205) + mi(MEDSCI_202) + mi(MEDSCI_204)) +
    bf(MEDSCI_203 | mi() ~ 1) + # ~ 1 uses the mean
    bf(MEDSCI_205 | mi() ~ 1) +
    bf(MEDSCI_202 | mi() ~ 1) +
    bf(MEDSCI_204 | mi() ~ 1),
  data = df_ug,
...
summary(fit_brms_301)
```

Same for MEDSCI 302

```{r}
# Check skewness for MEDSCI 302
library(moments)
skew_val <- skewness(df_ug$MEDSCI_302, na.rm = TRUE)
print(skew_val)
...
priors <- c(
  ...
  prior(normal(0, 5), class = "b"),
  prior(normal(0, 2), class = "sigma"),
  prior(normal(-1.2, 0.5), class = "alpha")
)


```

This gives different results comparing to BMA model:
```{r}
...
fit_brms_302 <- brm(
  bf(MEDSCI_302 ~ mi(MEDSCI_203) + mi(MEDSCI_205) + mi(MEDSCI_202) + mi(MEDSCI_204)) +
    bf(MEDSCI_203 | mi() ~ 1) + # ~ 1 uses the mean
    bf(MEDSCI_205 | mi() ~ 1) +
    bf(MEDSCI_202 | mi() ~ 1) +
    bf(MEDSCI_204 | mi() ~ 1),
